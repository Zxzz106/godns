...
image:
		# Build docker image
		go clean
		docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t Zxzz106/godns:${VERSION} . --push
		docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t Zxzz106/godns:latest . --push
release:
		# Clean
		go clean
		rm -rf *.gz
